package com.easy.aliplayer.view.gesturedialog;
...
public class SeekDialog extends BaseGestureDialog {
    ...
    public static int getTargetPosition(long duration, long currentPosition, long deltaPosition) {
        ...
        if (hours >= 1) {// 视频时长为1小时以上，小屏和全屏的手势滑动最长为视频时长的十分之一
            finalDeltaPosition = deltaPosition / 10;
        } else if (minutes > 30) {// 视频时长为31分钟－60分钟时，小屏和全屏的手势滑动最长为视频时长五分之一
            finalDeltaPosition = deltaPosition / 5;
        } else if (minutes > 10) { // 视频时长为11分钟－30分钟时，小屏和全屏的手势滑动最长为视频时长三分之一
            finalDeltaPosition = deltaPosition / 3;
        } else if (minutes > 3) {// 视频时长为4-10分钟时，小屏和全屏的手势滑动最长为视频时长二分之一
            finalDeltaPosition = deltaPosition / 2;
        } else {// 视频时长为1秒钟至3分钟时，小屏和全屏的手势滑动最长为视频结束
