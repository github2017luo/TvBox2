...
public class ControlView extends RelativeLayout implements ViewAction, ITheme {
    ...
    private void setViewListener() {
        //标题的返回按钮监听
        mTitlebarBackBtn.setOnClickListener(v -> {
            if (mOnBackClickListener != null) {
                mOnBackClickListener.onClick();
            }
        });
        //下载菜单监听
        mTitleDownload.setOnClickListener(v -> {
        ...
        //锁屏按钮监听
        mScreenLockBtn.setOnClickListener(v -> {
        ...
        // 录制按钮监听
        mScreenRecorder.setOnClickListener(v -> {
        ...
        //大小屏按钮监听
        mScreenModeBtn.setOnClickListener(v -> {
        ...
        SeekBar.OnSeekBarChangeListener seekBarChangeListener = new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser) {
                    ...
                    if (mAliyunScreenMode == AliyunScreenMode.Full) { //全屏状态.
                        mLargePositionText.setText(TimeFormater.formatMs(progress));
                    } else if (mAliyunScreenMode == AliyunScreenMode.Small) { //小屏状态
        ...
        mLargeChangeQualityBtn.setOnClickListener(v -> {
            ...
            if (mOnQualityBtnClickListener != null && mAliyunMediaInfo != null) {
                ...
                for (TrackInfo trackInfo : trackInfos) {
                    if (trackInfo.getType() == TrackInfo.Type.TYPE_VOD) { //清晰度
        ...
        // 更多按钮点击监听
        mTitleMore.setOnClickListener(v -> {
    ...
    private void updateChangeQualityBtn() {
        if (mLargeChangeQualityBtn != null) {
            Log.d(TAG, "mCurrentQuality = " + mCurrentQuality + " , isMts Source = " + isMtsSource + " , mForceQuality = " + mForceQuality);
    ...
    private void updateSmallInfoBar() {
        if (mAliyunScreenMode == AliyunScreenMode.Full) {
            ...
            if (mAliyunMediaInfo != null) {
                ...
                mSmallSeekbar.setMax(mAliyunMediaInfo.getDuration());
            } else {
    ...
    private void updateLargeInfoBar() {
        if (mAliyunScreenMode == AliyunScreenMode.Small) {
            ...
            if (mAliyunMediaInfo != null) {
                ...
                mLargeSeekbar.setMax(mAliyunMediaInfo.getDuration());
            } else {
